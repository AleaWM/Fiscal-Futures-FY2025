...
![](generated/FIGURE1.png){width=100%}
...
cat(readr::read_file("generated/TABLE1.html"))
...
# Step by Step Methodology and Exploration
...
```{r}
#| label: fig-tax-refunds-noabatement
#| fig-cap:  "Tax Refunds without FY23 Abatements"
...
```{r}
#| label: fig-includedrefunds
#| fig-cap: "Based on current fund coding in the fund_ab_in.xlsx file, most refund dollars are already being excluded by having `in_ff` = 0. Funds that have `in_ff` = 0 are excluded from revenue and expenditure sums when calculating the Fiscal Gap."
...
```{r}
#| label: fig-tax-refunds-revenueside
#| fig-cap: "Tax Refunds from Revenues. Includes only funds designated as Refund funds in fund descriptions."
...
```{r}
#| label: fig-pensions-POB
#| fig-cap: "Pension Expenditures"
...
```{r}
#| label: fig-drop-local-transfers
#| fig-cap: "Drop Transfers from State to Local Governments"
...
```{r}
#| label: coding-stimuluspackage-dollars
...
<!--- If there are NA rev types, it will cause the code to  break when binding dataframes together a few chunks lower  --->
...
#| tbl-cap: "Year totals without gap line"
#| tbl-cap-location: top
...
#| fig-cap: "Largest Groups"
#| fig-subcap: 
#| - "Largest Expenditures for FY2025"
#| - "Largest Revenue Sources for FY2025"
...
```{r}
#| label: fig-top3-ownsourcerevenues
#| fig-cap: "Top 3 Own-Source Revenues Sources"
#| fig-cap-location: top
...
salestax <- salestax |> left_join(adjustments, by = c("Year" = "year")) 
# 
...
# 2) GROWTH & 2-1-2 MOVING AVERAGE ----------------
sales <- salestax %>% 
rename(dollars = Dollars,
year = Year) |>
  mutate(
    growth = dollars/lag(dollars) - 1
  )

library(zoo)
library(patchwork)

# Weighted 5-term centered smoother with weights 2-1-2 (prev2, prev1, curr, next1, next2 = 2,1,0,1,2)
w212 <- c(2,1,0,1,2) / sum(c(2,1,0,1,2))
sales <- sales %>%
  mutate(
    ma212 = rollapply(growth, 5, function(x) sum(x * w212), fill = NA, align = "center")
  )

# 3) PLOTS ----------------------------------------
avg_nom <- mean(sales$growth, na.rm = TRUE)
avg_ma  <- mean(sales$ma212,  na.rm = TRUE)

p_points <- ggplot(sales, aes(year, growth)) +
  geom_point(size = 1.6) +
geom_line() + 
 # geom_smooth(se = FALSE, method = "loess", span = 0.6, linewidth = 0.7) +
  geom_hline(yintercept = avg_nom, linetype = "dashed") +
  scale_y_continuous(labels = percent) +
  labs(title = "Nominal growth (YoY)", x = NULL, y = "Growth rate") +
  theme_minimal(base_size = 12)

p_ma <- ggplot(sales, aes(year, ma212)) +
  geom_line(linewidth = 0.9) +
  geom_hline(yintercept = avg_ma, linetype = "dashed") +
  scale_y_continuous(labels = percent) +
  labs(title = "Nominal growth — 5-term 2-1-2 moving average", x = NULL, y = "Smoothed growth") +
  theme_minimal(base_size = 12)

p_sales <- p_points / p_ma +
  plot_annotation(
    title = "Illinois Sales Tax Growth (Nominal)",
    subtitle = "Top: YoY with LOESS; Bottom: centered 5-term 2-1-2 weighted moving average.\nDashed lines are series averages.",
    caption = "Source: Fiscal Futures Project"
  )
...
```{r}
# Assume 'CPI' exists; deflate to base year
...
mutate(
    ...
    ma212_real   = rollapply(r_growth, 5, function(x) sum(x * w212), fill = NA, align = "center")
  )
...
geom_line() + 
 ...
 # geom_smooth(se = FALSE, method = "loess", span = 0.6, linewidth = 0.7) +
  ...
  scale_y_continuous(labels = percent) +
  labs(title = "Real growth (YoY)", x = NULL, y = "Growth rate") +
  theme_minimal(base_size = 12)
...
p_ma_real <- ggplot(sales, aes(year, ma212_real)) +
  ...
  scale_y_continuous(labels = percent) +
  labs(title = "Real growth — 5-term 2-1-2 moving average", x = NULL, y = "Smoothed growth") +
  theme_minimal(base_size = 12)

p_sales4 <- (p_points | p_real) / (p_ma | p_ma_real) +
  plot_annotation(
    title = "Various Measures of Illinois Sales Tax Growth Rate",
    subtitle = "Dashed lines show sample averages; LOESS approximates Stata’s connected overlay.",
    caption = "Source: Fiscal Futures Project"
  )
print(p_ma_real)
print(p_sales4)

#ggsave("generated/Sales Tax/sales_tax_growth_4panel.png", p_sales4, width = 12, height = 8, dpi = 300)
#ggsave("generated/Sales Tax/sales_tax_growth_4panel.svg", p_sales4, width = 12, height = 8)
...
Key funds: Healthcare Provider Relieve (0793) and Hospiotal Provider (0346)
0365 is Health and Human Services Medicaid TR ? 
0740 is Medicaid Buy in Program

Healthcare provider taxes come from revenue source 0133.

2104 = Medicare Part D
2683 = MCO Provider Assessment --> Mostly goes to Healthcare Provider Relief Fund 0793 in HFS
2526 = Hospital Provider Fund (not used in fy2025)

0133 = Health Care Provider Tax --> Mostly goes to Hospital Provider Fund 0346
0137 = Health Care Prov-Hospital (not used in  fy2025)
0145 = IHFA Medicaid Provider (not used in fy2025)
...
ggplot()+
 ...
 labs(title = "Medicaid reimbursements and Medicaid expenditures", caption = "Medicaid expenditures include funds provided to medical providers.",
      x = element_blank()) +
  scale_y_billions(name = "Dollars") + 
